...
| 3 | Exact conservative-term enumeration and simultaneous Shannon size/depth for fixed `Q` | Closed form `2^(5*2^(r-1)-5) * 3^(3^r-3*2^r+3)`; one parameter-free compiler simultaneously gives `O(3^r/r)` size and `(3/2)r+O(r/log r)` depth via an independently audited Boolean R9; worst-case size `Theta(3^r/r)` and depth `Theta(r)` with almost-all lower bounds | High: exact fixed-clone census plus simultaneous order-optimal size/linear depth and a sharply narrowed coefficient interval | The specialization is already in closed-class or two-rail synthesis literature, or the Pixley/global-anchor/R9 audit fails |
...
| Worst-case and almost-all term-DAG depth | `MANUSCRIPT THEOREM`: `r-log_3(log r)-O(1) <= Delta_r(Q) <= (3/2)r+O(r/log r)`, hence `Theta(r)` | Exact R9: 4,608 Boolean, 9,216 relative-orientation, 177,147 two-plane `Q` checks; 135 composed tables/1,641 tuples; recurrence through 256; independent no-solver audit | Independent full proof review; close coefficient interval `[1,3/2]`; Lean; database-native two-rail/closed-class prior art |

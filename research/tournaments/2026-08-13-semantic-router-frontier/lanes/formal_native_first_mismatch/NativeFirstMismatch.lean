...
theorem addressOutcome_equal_iff :
  ...
  | n + 1, (targetDigit, targetTail), (physicalDigit, physicalTail) => by
      change (digitOutcome targetDigit physicalDigit).comp
        (addressOutcome n targetTail physicalTail) = .equal ↔
          ((targetDigit, targetTail) : Digit × Address n) =
            (physicalDigit, physicalTail)
      cases targetDigit <;> cases physicalDigit <;>
        simp [digitOutcome, Outcome.comp,
...
theorem roots_injective (n : Nat) :
  ...
  change root n left left = root n right left at hValue
  have hLeft := root_self n left
  have hRight := root_ne_two_of_ne n right left hDifferent
  apply hRight
  exact hValue.symm.trans hLeft

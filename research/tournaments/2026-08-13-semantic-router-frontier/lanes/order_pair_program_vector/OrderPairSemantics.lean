...
theorem upper_middle_eq_gain_right (prior : SegmentSummary) (target : Digit)
    (prefixDisjoint : (prior.eqFlag && prior.gain) = false) :
    upperBit (prior.comp (digitSummary target .middle)) =
      (prior.comp (digitSummary target .right)).gain := by
  rcases prior with ⟨prefixEqual, prefixGain⟩
...
theorem composePairTerms_correct {Var : Type} (environment : Var → Q)
  ...
  cases leftEqual <;> cases leftGain <;>
    ...
    simp_all [PairTerms.Realizes, composePairTerms, lossBit, QTerm.eval,
      qDisc, qBit, SegmentSummary.comp]
